...
printf "."
# Generate a 65536 byte 1KHz positive going pulse in .WAV format only.
data="\\xFF\\xFF\\x00\\x00\\x00\\x00\\x00\\x00"
> /tmp/pulse.wav
chmod 644 /tmp/pulse.wav
...
kbcommands()
{
	...
	#
	# **** Play an 8 second positive pulse burst. *****
	if [ "$kbinput" == "PULSE" ]
	then
		# Look for OSX 10.7+ "afplay" first...
		afplay /tmp/pulse.wav > /dev/null 2>&1
		# Then look for Linux "aplay"...
		aplay /tmp/pulse.wav > /dev/null 2>&1
	fi
	# ******* 8 second positive pulse burst end. ******
	#
...
commandhelp()
{
	...
	printf "PULSE<CR> .......... Generate a positive going pulse purely for amplifier tests.\n"
	printf "\n"
	read -p "Press <CR> to continue:- " -e kbinput
	printf "\x1B[H\x1B[0;36;44m"
	clrscn
...
# Preface:-
# ---------
# Original author:- Barry Walker, G0LCU.
#
# The whole project is Public Domain and you may do with it as you please.
# This manual is written primarily for the machine below. ANY mechanical and
# circuit alterations needed for other *NIX style platforms will have to be
# worked out by yourselves as the Macbook Pro 13" only has a single microphone
# input via a 4 pole 3.5mm jack socket.
#
# Enjoy...
#
...
# next program run. Also this AudioScope.Manual is saved inside the /tmp
# drawer/directory/folder. End of quick start...
#
# #########################################################
#
# The manual proper:-
...
# moment. Note that AudioScope.config is deleted so any saved parameters will
# be lost. The script is then restarted as though it was a first time run.
#
...
# for CygWin. the relevant files are generated at program startup.
#
...
# + 5.10V DC inside the yellow section of the command window.
#
# ALTDC<CR> This is UNFINISHED and is a TODO. Running it does nothing.
#
# PULSE<CR> - This command generates a positive going pulse for intital setting
# up of the vertical amplifier. unlike BURST<CR> there is no CygWin generator,
# BUT, as the file is in .WAV format just extract it and play it in Windows
# Media Player for any amplifier setup that may be needed in CygWin.
#
# #########################################################
#
# Part 2)
...
# -------------------------
#
# From this point on I have to assume that SOX or DSP is being used as DEMO
# does not need to be calibrated. Also that the script is NOT running...
#
# Using the FIRST circuit from the Manual OR the script the first part will be
# built to setup and test the timebase calibration.
# This also contains the parts list for the first build.
#
# Some snapshots and photos of the first build can be found here:-
#
# http://wisecracker.host22.com/public/cal_draw.jpg
# http://wisecracker.host22.com/public/cal_plot.jpg
# http://wisecracker.host22.com/public/mic_ear1.jpg
...
# 6) 1.2mm drill bit.
# 7) 4mm drill bit.
# 8) Lightweight cordless drill.
# 9) Small file set, various shapes.
# 10) Small screwdriver set, various ends.
# 11) Small vice, (OPTOINAL).
# 12) Crimp tool, (OPTIONAL).
# 13) A metal ruler, 12 inches/30 cms long.
#
# The First Build:-
# -----------------
#
...
# script. This is text mode but should still be easily readable.
# From now on be VERY careful...
#
# REFER TO THE DRAWING AND PHTOTOGRAPHS!!!
#
...
#    1KHz audio sinewave generator. IMPORTANT! READ the script first near the
#    start to decide which one is for you and to obtain the correct start and
#    finish points for this extension sub-script...
# 4) Now run the script and a second "xterm" window will be generated that will
#    run the 1KHz audio sinewave generator for about 8 seconds.
# 5) Inside the AudioScope window enter the command SOX<CR> to enable the
#    capture device, (DSP<CR> if you are just experimenting with Linux).
# 6) Open up the MBP System Preferences -> Sound window.
# 7) Connect the homebuilt hardware to the mic/ear socket WITHOUT the white
#    flylead being connected to the terminal block.
# 8) Allow a couple of seconds to settle and the Output will switch to
#    Headphones and the Input to External microphone.
# 9) Connect the white flylead to the terminal block as shown in the photos.
# 10) Inside the AudioScope window enter the command TEN<CR> for 10 continuous
#     captures.
# 11) Inside the 1KHz xterm window press <CR> and the generated signal will be
#     displayed inside the AudioScope window.
# 12) Whilst the scans and audio are running adjust the Input and Output levels
#     in the MBP System Preferences -> Sound window.
# 13) Repeat this a few times until a full scale _deflection_ is obtained. On
#     my machine the mic Input was set to about half way and the Headphones set
#     to about a quarter, (the "Use ambient noise reduction" is unticked).
...
# 15) Inside the AudioScope window enter the command ONE<CR>.
# 16) Inside the 1KHz xterm window press <CR> to generate an 8 second burst.
# 17) Inside the AudioScope window enter the command <CR> only for one scan.
# 18) Now enter the command HOLD<CR>. This places the AudioScope into storage
#     mode.
# 19) Use the command TBVAR<CR>, follow the on screen prompts and set the start
#     anywhere BUT the jump must be 3. This is 500uS/DIV, giving 2 divisions
#     per cycle.
# 20) The peaks or troughs should be spaced two divisions apart...
# 21) Well done! The first hurdle is over.
# 22) Inside the 1KHz xterm window press Ctrl-C until the window will closes
#     down.
# 23) Disconnect the home built hardware, AND, if required......
# 24) Inside the AudioScope window enter QUIT<CR> and quit the program.
# 25) Shut down the terminal and if need be the computer too.
#
# These two images are from my timebase calibration tests. My TBVAR values were
# scan start of 3 and scan jump of 3 to get a symmetrical appearance...
#
# http://wisecracker.host22.com/public/cal_plot.jpg
# http://wisecracker.host22.com/public/cal_draw.jpg
#
...
# Srews, washers, spacers, nuts and grommets as required.
# #########################################################
EOF
exit 0
